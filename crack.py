...
if not zipf:
    ...
    if( result ==0): #eg.0124
        ...
        if result==0:
            ...
            print("Sorry password not found" + str(t) + " possible combinations we tried " + str(dur) + "seconds")
        else:
            duration = endT-startT
            dur=time.strftime("%H:%M:%S", time.gmtime(duration))

            print("Congratulations , password found after " + str(t) +" tries , in  "+ str(dur) +" time")
